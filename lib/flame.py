...
from .lbs import batch_rodrigues, lbs, vertices2landmarks, rot_mat_to_euler
from .basetype import EasyDict
from .ops import to_tensor, to_np

MODEL_DIR = '../My3D/pretrained/FLAME/'


class FLAMEAPI(object):
    def __init__(self):
        self.flame = FLAME(
            EasyDict(
                batch_size=1,
                ...
                flame_model_path=f"{MODEL_DIR}/flame2023.pkl",
    ...
    def __call__(
        ...
        vertices, landmarks = self.flame(shape_params, expression_params,
            pose_params, neck_pose, eye_pose, transl)
        return vertices * self.scaling, landmarks * self.scaling
...
class FLAME(nn.Module):
    ...
    def __init__(self, config):
        ...
        with open(config.flame_model_path, "rb") as f:
            self.flame_model = EasyDict(**pickle.load(f, encoding="latin1"))
        self.NECK_IDX = 1
        ...
        with open(config.static_landmark_embedding_path, "rb") as f:
            static_embeddings = EasyDict(**pickle.load(f, encoding="latin1"))
